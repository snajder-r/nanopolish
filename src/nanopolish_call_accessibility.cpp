...
//---------------------------------------------------------
// Adapted 2020 by Rene Snajder (rene.snajder@dkfz-heidelberg.de)
//
// nanopolish_call_accessibility -- identify methylated bases
//
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
struct WatchStatus
{
    ...
    void update(const std::string& message)
    {
        ...
        fprintf(stderr, "\33[2K\r[call-accessibility] fast5 progress: %d/%d %.2lfs/fast5 [%s]", processed_fast5s, seconds_per_file, total_fast5s, message.c_str());
    }
};
...
// Getopt
//
#define SUBPROGRAM "call-accessibility"
...
"Classify regions as accessibility or not.\n"
"\n"
...
//79 = 0 = 'O'
//125 = 20 = '}'
inline char convert_LLR_to_char(const double llr, char offset){
    //Convert magnitude of llr to scale 0-20
    double converted = log(1.0 + abs(llr)) * 15.0;
    //Apply sign
    converted = ((llr>0.0) ? 1.0 : -1.0) * converted;
    //Offset, clip, and cast
    return (char) std::max<double>(33.0,std::min<double>(125.0, converted + (double)offset));
}
                                    ...
                                    int region_end)
{
    ...
    char* site_scores = new char[ref_end_pos-ref_start_pos+1];
    for(size_t i = 0; i < ref_end_pos-ref_start_pos; i++){
        site_scores[i] = 'O'; //ascii O is our 0, ironically (it's the center printable char)
    }
    site_scores[ref_end_pos-ref_start_pos] = '\0'; //Null-terminate
    ...
    for(size_t strand_idx = 0; strand_idx < NUM_STRANDS; ++strand_idx) {
        ...
        for(size_t i = opt::min_flank; i < ref_seq.size() - opt::min_flank; ++i) {
            ...
            int sub_start_pos = i - opt::min_flank;
            int sub_end_pos = i + opt::min_flank;
            ...
            double llr = methylated_score - unmethylated_score;
            site_scores[i] = convert_LLR_to_char(llr, site_scores[i]);
        } // for window
    } // for strands

    #pragma omp critical(call_accessibility_write)
    {
        fprintf(handles.site_writer, "%s\t%s\t%d\t%d\t", contig.c_str(), read_orientation.c_str(), ref_start_pos, ref_end_pos);
        fprintf(handles.site_writer, "%s\t%s\t\n", sr.read_name.c_str(), site_scores);
    }
}
...
void parse_call_accessibility_options(int argc, char** argv)
{
    bool die = false;
...
int call_accessibility_main(int argc, char** argv)
{
    call_accessibility::parse_call_accessibility_options(argc, argv);

    // load reference fai file
    faidx_t *fai = fai_load(call_accessibility::opt::genome_file.c_str());
    if(fai == NULL) {
        fprintf(stderr, "Error: could not open genome file: %s\n", call_accessibility::opt::genome_file.c_str());
        fprintf(stderr, "Please check the path is correct\n");
        exit(1);
    }

#ifndef H5_HAVE_THREADSAFE
    if(call_accessibility::opt::num_threads > 1) {
    ...
    omp_set_num_threads(call_accessibility::opt::num_threads);
    if(!call_accessibility::opt::watch_dir.empty()) {
        call_accessibility::run_watch_mode(fai);
    } else {
        call_accessibility::run_from_bam(fai);
    }
